...
def TongXunListView(request):
    ...
    return render(request, 'admin/通讯/通讯.html', context)
...
def TongXunDetailView(request, pk):
    ...
    return render(request, 'admin/通讯/通讯_detail.html', context)
...
def ShuPingListView(request):
    书评_list = 书评.objects.all().order_by('id')
    page = request.GET.get('page', 1)
    paginator = Paginator(书评_list, 20)
    try:
        书评_list = paginator.page(page)
    except PageNotAnInteger:
        书评_list = paginator.page(1)
    except EmptyPage:
        书评_list = paginator.page(paginator.num_pages)
    ...
    return render(request, 'admin/书评/书评.html', context)
...
class ShuPingAddView(generic.TemplateView):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('admin_task:书评'))
        else:
            messages.error(request, '书评添加失败请检查填表！')
            return render(request, 'admin/书评_add.html', {'form': form, 'content_title': 'shuping', 'task': 'content'})


def ShuPingDetailView(request, pk):
    ...
    return render(request, 'admin/书评/书评_detail.html', context)
